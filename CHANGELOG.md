...
- **Other LLM-related styles cleaned up for dark mode.**
    ...
    - `llm-form-result`, `llm-response-style`, `llm-structured-response`
      ...
      left untouched).
- **LLM chat surfaces flip in `ionic serve` too.** The previous fix wired
  the chat to `--ion-background-color` and `--ion-color-step-50`, which
  is the right call on a real device — Ionic's `theme/variables.scss`
...
### Enhancements
 ...
 - **llmChat custom sidebar drawer** - replaced `ion-menu` with a custom CSS sidebar drawer that works reliably in both normal and floating mode; backdrop overlay, slide-in animation, and proper z-index stacking
 - **llmChat floating panel fixes** - floating panel z-index raised to 99999 to ensure it covers the tab bar; inner container uses proper flex layout to prevent text input cutoff; `:host` display block with `overflow: hidden` prevents double scrollbar
 - **llmResponse event-driven loading** - when `enable_event_listener` is active, `formSubmitting` state persists beyond the HTTP response until the `EventListenerService` refreshes the page with new content, keeping the loading overlay visible while the LLM generates its response
 - **therapist dashboard unread counts** - fetches per-subject unread counts via `get_unread_counts` action and displays them as badges on each conversation; falls back to `unread_alerts` field
