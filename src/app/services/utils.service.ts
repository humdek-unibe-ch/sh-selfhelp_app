...
export class UtilsService {
    ...
    // ionic cordova build android --prod --release "--" --packageType=apk  // first
    // cordova build android --release "--" --packageType=apk  // second
    ...
    // Local test with ip and ssl then select the ip address, in my case https://84.75.194.2:8100
    ...
    // ng build --base-href "/SelfHelpMobilePreview/v3_0_0/" --deploy-url "/SelfHelpMobilePreview/v3_0_0/"

    private debugMode: boolean = true;

    constructor() { }

    public debugLog(text: string, obj: any): void {
        if (this.debugMode) {
            console.log('(debugLog)[' + format(new Date(), 'dd-MM-yyyy: HH:mm:ss.SSS') + '] ' + text, obj);
        }
    }
