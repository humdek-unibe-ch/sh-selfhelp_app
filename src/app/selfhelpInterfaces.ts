...
function convertUnifiedToStructuredResponse(unified: Record<string, unknown>): LlmStructuredResponse {
    ...
    const textBlocks = content['text_blocks'] as Array<{ type: string; content: string; style?: string }>;
...
/**
 * Check if a message contains any form (either legacy FormDefinition or StructuredResponse form)
 */
export function llmMessageHasForm(content: string): boolean {
    // Check for legacy form definition
    const legacyForm = parseLlmFormDefinition(content);
    if (legacyForm) return true;

    // Check for structured response with forms
    const structuredResponse = parseLlmStructuredResponse(content);
    if (structuredResponse?.content?.forms && structuredResponse.content.forms.length > 0) {
        return true;
    }
...
/**
 * Extract form definition from any message format
 */
export function extractLlmFormFromMessage(content: string): LlmFormDefinition | null {
    // Check for legacy form definition
    const legacyForm = parseLlmFormDefinition(content);
    ...
    const structuredResponse = parseLlmStructuredResponse(content);
    if (structuredResponse?.content?.forms && structuredResponse.content.forms.length > 0) {
        const form = structuredResponse.content.forms[0];
        return {
            type: 'form',
            title: form.title,
